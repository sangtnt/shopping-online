...
router.route("/getCatById/:catId").get((req, res)=>{
  ...
  connection.connect(function(err) {
    if (err) throw err;
    connection.query("SELECT * FROM category WHERE cat_id="+catId, function (err, result, fields) {
      res.json({category:result});
    });
  });
})
router.route("/delete/:catId").get((req, res)=>{
  ...
  connection.connect(function(err) {
    ...
    connection.query("DELETE FROM category WHERE cat_id="+catId, function (err, result, fields) {
...
router.route("/probycategory/:catId").get((req, res)=>{
  ...
  connection.connect(function(err) {
    ...
    connection.query(`SELECT * FROM product WHERE product.cat_id=${catId}`, function (err, result, fields) {
